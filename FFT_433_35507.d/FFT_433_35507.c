...
#include "FFT_433_35507.h"

/*Set n, q and N*/
static const FFTSHORT n = 433;
static const FFTSHORT q = 35507;
static const FFTSHORT N = 1024;

#include "FFT_433_35507_constants.c"
...
void slow_cyclic_convolution_433_35507(FFTSHORT z[433], FFTSHORT x[433], FFTSHORT y[433]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void cyclic_convolution_433_35507(FFTSHORT z[433], FFTSHORT x[433], FFTSHORT y[433]) {
	...
	FFT_forward_433_35507(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_433_35507(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 35425UL, q);	
	}
}
