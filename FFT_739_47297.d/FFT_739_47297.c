...
#include "FFT_739_47297.h"

/*Set n, q and N*/
static const FFTSHORT n = 739;
static const FFTSHORT q = 47297;
static const FFTSHORT N = 2048;

#include "FFT_739_47297_constants.c"
...
void slow_cyclic_convolution_739_47297(FFTSHORT z[739], FFTSHORT x[739], FFTSHORT y[739]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void cyclic_convolution_739_47297(FFTSHORT z[739], FFTSHORT x[739], FFTSHORT y[739]) {

	FFT_forward_739_47297(x);
	FFT_forward_739_47297(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_739_47297(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 47233UL, q);	
	}
}
