...
#include "FFT_512_12289_constants.c"
...
*/
void FFT_forward_512_12289(FFTSHORT x[512]) {
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	FFTSHORT t0,t1;
...
*/
void FFT_backward_512_12289(FFTSHORT x[512]) {
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	FFTSHORT t0,t1;
...
the fourier transform using decimation-in-time.
*/
void FFT_twisted_forward_512_12289(FFTSHORT x[512]) {
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	FFTSHORT t0,t1;
...
fourier transform is using decimation-in-frequency.
*/
void FFT_twisted_backward_512_12289(FFTSHORT x[512]) {
	FFTSHORT index, step;
	FFTSHORT i,j,m;
	FFTSHORT t0,t1;
...
void slow_cyclic_convolution_512_12289(FFTSHORT z[512], FFTSHORT x[512], FFTSHORT y[512]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void cyclic_convolution_512_12289(FFTSHORT z[512], FFTSHORT x[512], FFTSHORT y[512]) {

	FFT_forward_512_12289(x);
	FFT_forward_512_12289(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_512_12289(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 12265UL, q);	
	}
}


/*O(n^2) negacyclic convolution code for testing*/
void slow_negacyclic_convolution_512_12289(FFTSHORT z[512], FFTSHORT x[512], FFTSHORT y[512]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void negacyclic_convolution_512_12289(FFTSHORT z[512], FFTSHORT x[512], FFTSHORT y[512]) {

	FFT_twisted_forward_512_12289(x);
	FFT_twisted_forward_512_12289(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_twisted_backward_512_12289(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {	
		MUL_MOD(z[i], z[i], 12265UL, q);	
	}
