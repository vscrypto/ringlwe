...
#include "FFT_821_49261.h"

/*Set n, q and N*/
static const FFTSHORT n = 821;
static const FFTSHORT q = 49261;
static const FFTSHORT N = 2048;

#include "FFT_821_49261_constants.c"


/*
We use Bluestein's trick and integer convoution by CRT.
*/
void FFT_forward_821_49261(FFTSHORT x[821]) {
...
void FFT_backward_821_49261(FFTSHORT x[821]) {
	FFTSHORT x0[2048], x1[2048];
	FFTSHORT i;
	FFTLONG x_crt[2048];
	const FFTSHORT q0 = 8816641, Ninvq0 = 8812336;	
...
void slow_cyclic_convolution_821_49261(FFTSHORT z[821], FFTSHORT x[821], FFTSHORT y[821]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void cyclic_convolution_821_49261(FFTSHORT z[821], FFTSHORT x[821], FFTSHORT y[821]) {

	FFT_forward_821_49261(x);
	FFT_forward_821_49261(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_821_49261(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 49201UL, q);	
	}
}
