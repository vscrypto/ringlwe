...
static const FFTSHORT q = 17633281;


#include "FFT_2048_17633281_constants.c"
...
void cyclic_convolution_2048_17633281(FFTSHORT z[2048], FFTSHORT x[2048], FFTSHORT y[2048]) {

	FFT_forward_2048_17633281(x);
	FFT_forward_2048_17633281(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_2048_17633281(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 17624671UL, q);	
	}
}
