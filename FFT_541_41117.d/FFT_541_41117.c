...
#include "FFT_541_41117.h"

/*Set n, q and N*/
static const FFTSHORT n = 541;
static const FFTSHORT q = 41117;
static const FFTSHORT N = 2048;

#include "FFT_541_41117_constants.c"
...
void slow_cyclic_convolution_541_41117(FFTSHORT z[541], FFTSHORT x[541], FFTSHORT y[541]) {
	FFTSHORT i,j,t;
	for (i = 0; i < n; ++i) {
		z[i] = 0;
		for (j = 0; j <= i; ++j) {
...
void cyclic_convolution_541_41117(FFTSHORT z[541], FFTSHORT x[541], FFTSHORT y[541]) {
	...
	FFT_forward_541_41117(x);
	FFT_forward_541_41117(y);
	
	FFTSHORT i;
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], x[i], y[i], q);
	}
	FFT_backward_541_41117(z);
	/*Multiply by n^-1 mod q*/
	for (i = 0; i < n; ++i) {
		MUL_MOD(z[i], z[i], 41041UL, q);	
	}
}
